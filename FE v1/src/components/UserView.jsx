...
export function UserView() {
  ...
  }, [token, documentName, metaData]);

  const getAllPayabale = () => {
    axios
      .get(`${baseUrl}/master/files-payable`, {
        headers: {
          Authorization: `${token}`,
        },
      })
  ...
  const getAllReceivable = () => {
    axios
      .get(`${baseUrl}/master/files-receivable`, {
        headers: {
          Authorization: `${token}`, // Format yang benar
        },
      })
      .then((res) => {
        setMetaData(res.data.data || []);
  ...
  const getAllDocumentName = () => {
    axios
      .get(`${baseUrl}/master/document-name`, {
        headers: {
          Authorization: `${token}`, // Format yang benar
        },
      })
    ...
    ? metaData
        ...
        .sort((a, b) => {
          const extractNumber = (filename) => {
            const match = filename?.match(/^(\d+)/); // Ambil angka awal
  ...
  const deletePDF = async (data) => {
    try {
      const res = await axios.delete(`${baseUrl}/master/delete-file`, {
        headers: {
          Authorization: `${token}`, // Format yang benar
        },
        data,
  ...
  const getFilePDF = async (filePath) => {
    try {
      const response = await axios.post(`${baseUrl}/master/file-url/`, {
        headers: {
          Authorization: `${token}`, // Format yang benar
        },
        filePath: filePath,
    ...
    <>
      {userdData ? (
          ...
          <div className="container-fluid p-5">
            ...
            <Card>
              <div className="flex md:flex-row flex-col items-center">
                ...
                <div className="font-bold text-4xl ms-2">DOCUMENTS FINANCE</div>
                <Select
                  id="documentName"
                ...
                {userdData.role_id === 1 || userdData.role_id === 3 ? (
                  <button
                    onClick={() => handleModal()}
                              ...
                              <TableCell>
                                <div className="buttonAction flex">
                                  ...
                                  userdData.role_id === 3 ? (
                                    <div className="btn">
                                      <button
                                        className="bg-green-500 mx-2  hover:bg-green-600 text-white px-2 py-2 rounded-lg"
                                        onClick={() => handleModalEdit(value)}
                                      >
                                        <img
                                          src="/icon/edit.svg"
        ...
        <div className="container-fluid h-screen ">
          <span>SILAHKAN LOGIN DENGAN BENAR!</span>
          <button
            onClick={() => {
